...
class NSELiveSession:
    ...
    def fetch_option_chain(self, symbol: str = 'NIFTY', symbol_type: str = 'INDEX') -> Dict[str, Any]:
        ...
        # Re-init cookie if not initialized or stale (> 5 mins)
        if not self.cookies_initialized or (time.time() - self.last_init_time > 300):
            self.init_session()
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                raise HTTPException(status_code=res.status_code, detail=f"NSE API returned status code {res.status_code}")
        except HTTPException:
            raise
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Error connecting to NSE: {str(e)}")
